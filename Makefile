...
# `make eval-... SKIP_COMPLETED=1` makes eval_accuracy.py skip any task whose
# result is already complete in the per-model output JSON (no prefetch, no
# server startup if all tasks are cached).  Default empty -> always re-run
# (use this to gather more samples by bumping LIMIT, or just to refresh).
# Exported so the script picks it up via os.environ — no need to thread a flag
# through each of the 25 eval-* recipes.
SKIP_COMPLETED ?=
...
GEMMA_Q8_MODEL     ?= $(GEMMA_DIR)/$(GEMMA_Q8_FILE)

GEMMA_Q4_QUANT     := Q4_K_M
GEMMA_Q4_FILE      := gemma-2-2b-it-$(GEMMA_Q4_QUANT).gguf
GEMMA_Q4_MODEL     ?= $(GEMMA_DIR)/$(GEMMA_Q4_FILE)

GEMMA_Q2_QUANT     := Q2_K
GEMMA_Q2_FILE      := gemma-2-2b-it-$(GEMMA_Q2_QUANT).gguf
GEMMA_Q2_MODEL     ?= $(GEMMA_DIR)/$(GEMMA_Q2_FILE)
...
.PHONY: help \
        ...
        gemma-q8-model gemma-q8-verify gemma-q4-model gemma-q4-verify gemma-q2-model gemma-q2-verify gemma-clean \
        benchmark-bitnet benchmark-qwen-q8 benchmark-qwen-q4 benchmark-qwen-q2 benchmark-gemma-q8 benchmark-gemma-q4 benchmark-gemma-q2 benchmark \
        benchmark-qwen-q8-on-bitnet-fork \
        benchmark-threads-bitnet benchmark-threads-qwen-q8 benchmark-threads-qwen-q4 \
        benchmark-threads-qwen-q2 benchmark-threads-gemma-q8 benchmark-threads-gemma-q4 benchmark-threads-gemma-q2 benchmark-threads \
        plots marginal-energy \
        smoke-test smoke-test-bitnet smoke-test-qwen-q8 smoke-test-qwen-q4 smoke-test-qwen-q2 smoke-test-gemma-q8 smoke-test-gemma-q4 smoke-test-gemma-q2 \
        system-cards system-cards-bitnet system-cards-qwen system-cards-cloud \
        eval-arc-easy-bitnet eval-arc-easy-qwen-q8 eval-arc-easy-qwen-q4 eval-arc-easy-qwen-q2 eval-arc-easy-gemma-q8 eval-arc-easy-gemma-q4 eval-arc-easy-gemma-q2 eval-arc-easy \
        eval-arc-challenge-bitnet eval-arc-challenge-qwen-q8 eval-arc-challenge-qwen-q4 eval-arc-challenge-qwen-q2 eval-arc-challenge-gemma-q8 eval-arc-challenge-gemma-q4 eval-arc-challenge-gemma-q2 eval-arc-challenge \
        eval-mmlu-bitnet eval-mmlu-qwen-q8 eval-mmlu-qwen-q4 eval-mmlu-qwen-q2 eval-mmlu-gemma-q8 eval-mmlu-gemma-q4 eval-mmlu-gemma-q2 eval-mmlu \
        eval-winogrande-bitnet eval-winogrande-qwen-q8 eval-winogrande-qwen-q4 eval-winogrande-qwen-q2 eval-winogrande-gemma-q8 eval-winogrande-gemma-q4 eval-winogrande-gemma-q2 eval-winogrande \
        eval-hellaswag-bitnet eval-hellaswag-qwen-q8 eval-hellaswag-qwen-q4 eval-hellaswag-qwen-q2 eval-hellaswag-gemma-q8 eval-hellaswag-gemma-q4 eval-hellaswag-gemma-q2 eval-hellaswag \
        eval-accuracy-bitnet eval-accuracy-qwen-q8 eval-accuracy-qwen-q4 eval-accuracy-qwen-q2 eval-accuracy-gemma-q8 eval-accuracy-gemma-q4 eval-accuracy-gemma-q2 eval-accuracy \
...
help:
	...
	@echo ""
	@echo "Gemma 2 2B Instruct baseline (second model family, reuses qwen-build):"
	@echo "  gemma-q8-model                Download Gemma-2-2B-it Q8_0 GGUF from $(GEMMA_REPO)"
	@echo "  gemma-q8-verify               Quick inference smoke-test with Gemma-2-2B-it Q8_0"
	@echo "  gemma-q4-model                Download Gemma-2-2B-it Q4_K_M GGUF (reuses qwen-build)"
	@echo "  gemma-q4-verify               Quick inference smoke-test with Gemma-2-2B-it Q4_K_M"
	@echo "  gemma-q2-model                Download Gemma-2-2B-it Q2_K GGUF (reuses qwen-build)"
	@echo "  gemma-q2-verify               Quick inference smoke-test with Gemma-2-2B-it Q2_K"
	@echo ""
	...
	@echo "  benchmark-gemma-q8            Gemma-2-2B-it Q8_0 inference benchmark"
	@echo "  benchmark-gemma-q4            Gemma-2-2B-it Q4_K_M inference benchmark"
	@echo "  benchmark-gemma-q2            Gemma-2-2B-it Q2_K inference benchmark"
	@echo "  benchmark                     All seven models"
	...
	@echo "  eval-arc-easy-gemma-q8        Gemma-2-2B-it Q8_0 ARC-Easy eval"
	@echo "  eval-arc-easy-gemma-q4        Gemma-2-2B-it Q4_K_M ARC-Easy eval"
	@echo "  eval-arc-easy-gemma-q2        Gemma-2-2B-it Q2_K ARC-Easy eval"
	@echo "  eval-arc-easy                 All seven models ARC-Easy"
	...
	@echo "  eval-arc-challenge-gemma-q8   Gemma-2-2B-it Q8_0 ARC-Challenge eval"
	@echo "  eval-arc-challenge-gemma-q4   Gemma-2-2B-it Q4_K_M ARC-Challenge eval"
	@echo "  eval-arc-challenge-gemma-q2   Gemma-2-2B-it Q2_K ARC-Challenge eval"
	@echo "  eval-arc-challenge            All seven models ARC-Challenge"
	...
	@echo "  eval-mmlu-gemma-q8            Gemma-2-2B-it Q8_0 MMLU 5-shot eval"
	@echo "  eval-mmlu-gemma-q4            Gemma-2-2B-it Q4_K_M MMLU 5-shot eval"
	@echo "  eval-mmlu-gemma-q2            Gemma-2-2B-it Q2_K MMLU 5-shot eval"
	@echo "  eval-mmlu                     All seven models MMLU"
	...
	@echo "  eval-winogrande-gemma-q8      Gemma-2-2B-it Q8_0 WinoGrande eval"
	@echo "  eval-winogrande-gemma-q4      Gemma-2-2B-it Q4_K_M WinoGrande eval"
	@echo "  eval-winogrande-gemma-q2      Gemma-2-2B-it Q2_K WinoGrande eval"
	@echo "  eval-winogrande               All seven models WinoGrande"
	...
	@echo "  eval-hellaswag-gemma-q8       Gemma-2-2B-it Q8_0 HellaSwag eval"
	@echo "  eval-hellaswag-gemma-q4       Gemma-2-2B-it Q4_K_M HellaSwag eval"
	@echo "  eval-hellaswag-gemma-q2       Gemma-2-2B-it Q2_K HellaSwag eval"
	@echo "  eval-hellaswag                All seven models HellaSwag"
	...
	@echo "  eval-accuracy-gemma-q8        All tasks, Gemma-2-2B-it Q8_0 only"
	@echo "  eval-accuracy-gemma-q4        All tasks, Gemma-2-2B-it Q4_K_M only"
	@echo "  eval-accuracy-gemma-q2        All tasks, Gemma-2-2B-it Q2_K only"
	@echo "  eval-accuracy                 All tasks, all seven models"
	...
	@echo "  smoke-test                    Verify scripts produce expected outputs (all seven models)"
	...
	@echo "  smoke-test-gemma-q8           Verify Gemma-2-2B-it Q8_0 inference only"
	@echo "  smoke-test-gemma-q4           Verify Gemma-2-2B-it Q4_K_M inference only"
	@echo "  smoke-test-gemma-q2           Verify Gemma-2-2B-it Q2_K inference only"
	@echo ""
	...
	@echo "  SKIP_COMPLETED=1           Skip eval tasks already complete in the per-model JSON (default: re-run)"
...
GEMMA_Q8_BENCH_OUT   ?= results/gemma_q8_step_metrics.csv
GEMMA_Q4_BENCH_OUT   ?= results/gemma_q4_step_metrics.csv
GEMMA_Q2_BENCH_OUT   ?= results/gemma_q2_step_metrics.csv
UBATCH               ?= 128
...
benchmark-gemma-q8:
	$(POETRY) run python scripts/metrics_tracker.py \
		--llama-dir $(QWEN_LLAMACPP_DIR) \
		--model $(GEMMA_Q8_MODEL) \
		--out $(GEMMA_Q8_BENCH_OUT) \
		--threads $(THREADS) \
		--ubatch $(UBATCH)
...
benchmark-gemma-q4:
	$(POETRY) run python scripts/metrics_tracker.py \
		--llama-dir $(QWEN_LLAMACPP_DIR) \
		--model $(GEMMA_Q4_MODEL) \
		--out $(GEMMA_Q4_BENCH_OUT) \
		--threads $(THREADS) \
		--ubatch $(UBATCH)
...
benchmark-gemma-q2:
	$(POETRY) run python scripts/metrics_tracker.py \
		--llama-dir $(QWEN_LLAMACPP_DIR) \
		--model $(GEMMA_Q2_MODEL) \
		--out $(GEMMA_Q2_BENCH_OUT) \
		--threads $(THREADS) \
		--ubatch $(UBATCH)

benchmark: benchmark-bitnet benchmark-qwen-q8 benchmark-qwen-q4 benchmark-qwen-q2 benchmark-gemma-q8 benchmark-gemma-q4 benchmark-gemma-q2
...
GEMMA_Q8_THREAD_SWEEP_OUT ?= results/gemma_q8_thread_sweep.csv
GEMMA_Q4_THREAD_SWEEP_OUT ?= results/gemma_q4_thread_sweep.csv
GEMMA_Q2_THREAD_SWEEP_OUT ?= results/gemma_q2_thread_sweep.csv
...
benchmark-threads-gemma-q8:
	$(POETRY) run python scripts/metrics_tracker.py --llama-dir $(QWEN_LLAMACPP_DIR) --model $(GEMMA_Q8_MODEL) --threads 1 --out $(GEMMA_Q8_THREAD_SWEEP_OUT) --no-energy
	$(POETRY) run python scripts/metrics_tracker.py --llama-dir $(QWEN_LLAMACPP_DIR) --model $(GEMMA_Q8_MODEL) --threads 2 --out $(GEMMA_Q8_THREAD_SWEEP_OUT) --no-energy
	$(POETRY) run python scripts/metrics_tracker.py --llama-dir $(QWEN_LLAMACPP_DIR) --model $(GEMMA_Q8_MODEL) --threads 4 --out $(GEMMA_Q8_THREAD_SWEEP_OUT) --no-energy
	$(POETRY) run python scripts/metrics_tracker.py --llama-dir $(QWEN_LLAMACPP_DIR) --model $(GEMMA_Q8_MODEL) --threads 6 --out $(GEMMA_Q8_THREAD_SWEEP_OUT) --no-energy

benchmark-threads-gemma-q4:
	$(POETRY) run python scripts/metrics_tracker.py --llama-dir $(QWEN_LLAMACPP_DIR) --model $(GEMMA_Q4_MODEL) --threads 1 --out $(GEMMA_Q4_THREAD_SWEEP_OUT) --no-energy
	$(POETRY) run python scripts/metrics_tracker.py --llama-dir $(QWEN_LLAMACPP_DIR) --model $(GEMMA_Q4_MODEL) --threads 2 --out $(GEMMA_Q4_THREAD_SWEEP_OUT) --no-energy
	$(POETRY) run python scripts/metrics_tracker.py --llama-dir $(QWEN_LLAMACPP_DIR) --model $(GEMMA_Q4_MODEL) --threads 4 --out $(GEMMA_Q4_THREAD_SWEEP_OUT) --no-energy
	$(POETRY) run python scripts/metrics_tracker.py --llama-dir $(QWEN_LLAMACPP_DIR) --model $(GEMMA_Q4_MODEL) --threads 6 --out $(GEMMA_Q4_THREAD_SWEEP_OUT) --no-energy

benchmark-threads-gemma-q2:
	$(POETRY) run python scripts/metrics_tracker.py --llama-dir $(QWEN_LLAMACPP_DIR) --model $(GEMMA_Q2_MODEL) --threads 1 --out $(GEMMA_Q2_THREAD_SWEEP_OUT) --no-energy
	$(POETRY) run python scripts/metrics_tracker.py --llama-dir $(QWEN_LLAMACPP_DIR) --model $(GEMMA_Q2_MODEL) --threads 2 --out $(GEMMA_Q2_THREAD_SWEEP_OUT) --no-energy
	$(POETRY) run python scripts/metrics_tracker.py --llama-dir $(QWEN_LLAMACPP_DIR) --model $(GEMMA_Q2_MODEL) --threads 4 --out $(GEMMA_Q2_THREAD_SWEEP_OUT) --no-energy
	$(POETRY) run python scripts/metrics_tracker.py --llama-dir $(QWEN_LLAMACPP_DIR) --model $(GEMMA_Q2_MODEL) --threads 6 --out $(GEMMA_Q2_THREAD_SWEEP_OUT) --no-energy

benchmark-threads: benchmark-threads-bitnet benchmark-threads-qwen-q8 benchmark-threads-qwen-q4 benchmark-threads-qwen-q2 benchmark-threads-gemma-q8 benchmark-threads-gemma-q4 benchmark-threads-gemma-q2
...
GEMMA_Q8_ACC_OUT ?= results/accuracy_results_gemma_q8.json
GEMMA_Q4_ACC_OUT ?= results/accuracy_results_gemma_q4.json
GEMMA_Q2_ACC_OUT ?= results/accuracy_results_gemma_q2.json
...
eval-arc-easy-gemma-q8:
	$(POETRY) run python scripts/eval_accuracy.py \
		...
		--ubatch $(EVAL_UBATCH) \
		--limit $(LIMIT) \
		--out $(GEMMA_Q8_ACC_OUT) \
		--start-server

eval-arc-easy-gemma-q4:
	$(POETRY) run python scripts/eval_accuracy.py \
		--task arc_easy \
		--llama-dir $(QWEN_LLAMACPP_DIR) \
		--model $(GEMMA_Q4_MODEL) \
		...
		--ubatch $(EVAL_UBATCH) \
		--limit $(LIMIT) \
		--out $(GEMMA_Q4_ACC_OUT) \
		--start-server

eval-arc-easy-gemma-q2:
	$(POETRY) run python scripts/eval_accuracy.py \
		--task arc_easy \
		--llama-dir $(QWEN_LLAMACPP_DIR) \
		--model $(GEMMA_Q2_MODEL) \
		...
		--ubatch $(EVAL_UBATCH) \
		--limit $(LIMIT) \
		--out $(GEMMA_Q2_ACC_OUT) \
		--start-server

eval-arc-easy: eval-arc-easy-bitnet eval-arc-easy-qwen-q8 eval-arc-easy-qwen-q4 eval-arc-easy-qwen-q2 eval-arc-easy-gemma-q8 eval-arc-easy-gemma-q4 eval-arc-easy-gemma-q2
...
eval-arc-challenge: eval-arc-challenge-bitnet eval-arc-challenge-qwen-q8 eval-arc-challenge-qwen-q4 eval-arc-challenge-qwen-q2 eval-arc-challenge-gemma-q8 eval-arc-challenge-gemma-q4 eval-arc-challenge-gemma-q2
...
eval-mmlu: eval-mmlu-bitnet eval-mmlu-qwen-q8 eval-mmlu-qwen-q4 eval-mmlu-qwen-q2 eval-mmlu-gemma-q8 eval-mmlu-gemma-q4 eval-mmlu-gemma-q2
...
eval-winogrande: eval-winogrande-bitnet eval-winogrande-qwen-q8 eval-winogrande-qwen-q4 eval-winogrande-qwen-q2 eval-winogrande-gemma-q8 eval-winogrande-gemma-q4 eval-winogrande-gemma-q2
...
eval-hellaswag: eval-hellaswag-bitnet eval-hellaswag-qwen-q8 eval-hellaswag-qwen-q4 eval-hellaswag-qwen-q2 eval-hellaswag-gemma-q8 eval-hellaswag-gemma-q4 eval-hellaswag-gemma-q2
...
eval-accuracy-gemma-q8: eval-arc-easy-gemma-q8 eval-arc-challenge-gemma-q8 eval-mmlu-gemma-q8 eval-winogrande-gemma-q8 eval-hellaswag-gemma-q8

eval-accuracy-gemma-q4: eval-arc-easy-gemma-q4 eval-arc-challenge-gemma-q4 eval-mmlu-gemma-q4 eval-winogrande-gemma-q4 eval-hellaswag-gemma-q4

eval-accuracy-gemma-q2: eval-arc-easy-gemma-q2 eval-arc-challenge-gemma-q2 eval-mmlu-gemma-q2 eval-winogrande-gemma-q2 eval-hellaswag-gemma-q2

eval-accuracy: eval-accuracy-bitnet eval-accuracy-qwen-q8 eval-accuracy-qwen-q4 eval-accuracy-qwen-q2 eval-accuracy-gemma-q8 eval-accuracy-gemma-q4 eval-accuracy-gemma-q2
...
smoke-test-gemma-q8:
	$(POETRY) run python scripts/smoke_test.py gemma-q8

smoke-test-gemma-q4:
	$(POETRY) run python scripts/smoke_test.py gemma-q4

smoke-test-gemma-q2:
	$(POETRY) run python scripts/smoke_test.py gemma-q2
...
# Gemma 2 2B Instruct — Q8_0 / Q4_K_M / Q2_K GGUFs from bartowski, all
# reuse the upstream qwen-build (Gemma-2 architecture is supported by
# upstream llama.cpp out of the box).  Stored under $(GEMMA_DIR) to keep
# model families separated on disk.
gemma-q8-model:
	hf download $(GEMMA_REPO) $(GEMMA_Q8_FILE) \
		--local-dir $(GEMMA_DIR)

gemma-q8-verify:
	"$(QWEN_CLI)" -m $(GEMMA_Q8_MODEL) -p "What is 2+2?" -n 32 -t $(THREADS) --single-turn

gemma-q4-model:
	hf download $(GEMMA_REPO) $(GEMMA_Q4_FILE) \
		--local-dir $(GEMMA_DIR)
...
gemma-q2-verify:
	"$(QWEN_CLI)" -m $(GEMMA_Q2_MODEL) -p "What is 2+2?" -n 32 -t $(THREADS) --single-turn
...
define run_remote_benchmark
	...
	$(AWS_SSH) $(AWS_USER)@$2 '\
	    ...
	    docker run --rm \
	        ...
	        -e GEMMA_Q8_BENCH_OUT=results/out/gemma_q8_step_metrics.csv \
	        -e GEMMA_Q4_BENCH_OUT=results/out/gemma_q4_step_metrics.csv \
	        -e GEMMA_Q2_BENCH_OUT=results/out/gemma_q2_step_metrics.csv \
