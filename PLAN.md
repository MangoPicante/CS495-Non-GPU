...
└── results/
    ...
    ├── accuracy_results_gemma_q2.json    # (in-progress checkpoint — eval mid-MMLU)
...
**Gemma-2-2B-it Q8_0 and Q4_K_M tie at the top** of project mean accuracy
at **63.41% / 63.42%** — a striking quantization-invariance result.  Q4_K_M
matches Q8_0 *exactly* on ARC-Easy (73.0), ARC-Challenge (52.0),
WinoGrande (70.0), HellaSwag (64.0), and within sampling noise on MMLU
(58.11 vs 58.07).  Q4 buys the same accuracy at ~50% the weight storage
and +50% throughput.  Both rows lead the project by +1.7pt over BitNet's
mean and +2.3pt over Qwen Q8.  The cost: Gemma is the slowest model in
the project at every quant level (9.5 / 14.2 / 18.4 tok/s vs Qwen 15.1 /
24.9 / 32.5) and the heaviest at Q8/Q4 (~2.7 GB RSS, 2.2× BitNet).
Gemma's 2B parameter count vs Qwen 1.5B / BitNet 2B-ternary explains the
consistent throughput/memory gap.  BitNet still wins WinoGrande (75.2 vs
Gemma 70.0); Qwen Q8 still wins MMLU (62.28 vs Gemma 58.07).  Gemma
**Q2_K** drops RSS to 1,293 MB (under half of Q4) while throughput climbs
to 18.4 tok/s; accuracy is mid-flight — ARC tasks matched Q8/Q4 again
(73 / 52), MMLU partial (~26/57 subjects).
...
- [~] **Partial.** Accuracy evaluations underway for the three Gemma
      quants at `LIMIT=100` (cap chosen to keep wall-clock manageable —
      MMLU alone takes ~8.6 hr at Gemma Q8 throughput).  Gemma Q8 is
      complete and lands at 63.41% mean accuracy (project headline
      high).  Gemma Q4's MMLU is mid-flight and matched Q8 exactly on
      both ARC tasks (73.0 / 52.0).  Gemma Q2 queued next; Qwen Q4 and
      Q2 evals still owed.  Once everything lands, `accuracy_eval_cost.png`
      and `cloud_cost_accuracy.png` will fill in all seven models and
      the `_accuracy_scatter` Qwen Q4→Q2 + Gemma Q8→Q4→Q2 connectors
      activate.  Workflow improvement: `eval_accuracy.py` now reads
      `SKIP_COMPLETED=1` (env var or `--skip-completed` flag) so resuming
      a crashed/interrupted run skips finished tasks; also reconfigures
      stdout/stderr to UTF-8 on Windows so the end-of-run summary table
      (which uses `Δ` for the ours-vs-paper gap column) doesn't crash on
      cp1252 terminals after data is already saved.
