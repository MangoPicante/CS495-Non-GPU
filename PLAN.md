...
| Gemma-2-2B-it Q2_K GGUF | `bartowski/gemma-2-2b-it-GGUF` (`gemma-2-2b-it-Q2_K.gguf`, ~1.2 GiB) | `../Models/Gemma/` |
...
└── results/
    ...
    ├── gemma_q8_step_metrics.csv    # Gemma-2-2B-it Q8_0 llama-bench results   (pending)
    ├── gemma_q4_step_metrics.csv    # Gemma-2-2B-it Q4_K_M llama-bench results (pending)
    ├── gemma_q2_step_metrics.csv    # Gemma-2-2B-it Q2_K llama-bench results   (pending)
    ...
    ├── accuracy_results_gemma_q8.json    # (pending — eval not yet run)
    ├── accuracy_results_gemma_q4.json    # (pending — eval not yet run)
    ├── accuracy_results_gemma_q2.json    # (pending — eval not yet run)
...
**Qwen Q2_K** currently leads on throughput / RSS / cost but lacks
accuracy data; the three **Gemma-2-2B-it** rows are pending all metrics
— runs scheduled before the
...
- [ ] Run benchmarks for Qwen Q2_K and Gemma Q8/Q4/Q2 in the local Linux
      ...
      four models).
- [ ] Run accuracy evaluations for Qwen Q2_K and the three Gemma quants
      (no `accuracy_results_qwen_q2.json` / `accuracy_results_gemma_*.json`
      yet; `accuracy_eval_cost.png` and `cloud_cost_accuracy.png` still
      show three models instead of seven, and the `_accuracy_scatter`
      scatters miss the Q4→Q2 connector for Qwen and the Q8→Q4→Q2 chain
      for Gemma that the new code is wired to draw once data lands).
