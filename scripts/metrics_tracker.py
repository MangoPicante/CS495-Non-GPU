...
RESULTS_CSV = Path(__file__).parent.parent / "results" / "step_metrics.csv"
...
def parse_args():
    ...
    p.add_argument("--bitnet-dir", required=True, help="Path to built BitNet repo")
...
def find_llama_bench(bitnet_dir: Path) -> Path:
    ...
    if candidate2.exists():
        return candidate2
    sys.exit(f"llama-bench not found under {bitnet_dir}/build. Run 'make bitnet-build' first.")


def run_bench(llama_bench: Path, model: Path, n_prompt: int, n_gen: int, threads: int):
    cmd = [
        str(llama_bench),
        "-m", str(model),
        "-p", str(n_prompt),
